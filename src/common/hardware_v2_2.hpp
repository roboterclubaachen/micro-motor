...
struct Motor
{
	...
	configure(Phase phase, PhaseConfig phaseOutputConfig)
	{
		switch (phaseOutputConfig)
		{
			case PhaseConfig::HiZ:
				MotorTimer::configureOutputChannel(
					static_cast<uint32_t>(phase) + 1, MotorTimer::OutputCompareMode::ForceActive,
					MotorTimer::PinState::Enable, MotorTimer::OutputComparePolarity::ActiveLow,
					MotorTimer::PinState::Enable, MotorTimer::OutputComparePolarity::ActiveHigh,
					MotorTimer::OutputComparePreload::Disable);
				break;
			case PhaseConfig::Pwm:
				MotorTimer::configureOutputChannel(
					static_cast<uint32_t>(phase) + 1, MotorTimer::OutputCompareMode::Pwm,
					MotorTimer::PinState::Enable, MotorTimer::OutputComparePolarity::ActiveHigh,
					MotorTimer::PinState::Enable, MotorTimer::OutputComparePolarity::ActiveHigh,
					MotorTimer::OutputComparePreload::Disable);
				break;
	...
	initializeHall()
	{
		static_assert(HallPort::number_of_ports == 1,
					  "Hall pins must be at the same port to guarantee atomic read operations.");
	...
	initialize()
	{
		...
		MotorTimer::connect<PhaseUN::Ch1n, PhaseVN::Ch2n, PhaseWN::Ch3n, PhaseUP::Ch1, PhaseVP::Ch2,
							PhaseWP::Ch3>();
		initializeHall();
	}
};
...
inline void
setCurrentLimit(uint16_t limit)
{
	// Limit to 12 bit
...
initialize(CompBase::Hysteresis hysteresis = CompBase::Hysteresis::NoHysteresis)
{
	...
	CompV::initialize(CompV::InvertingInput::Dac3Ch2, CompV::NonInvertingInput::GpioB0, hysteresis);
	...
	TIM1->AF1 |= (TIM1_AF1_BKINE | TIM1_AF1_BKCMP3E | TIM1_AF1_BKCMP2E | TIM1_AF1_BKCMP4E);
	...
	AdcU::initialize(AdcU::ClockMode::SynchronousPrescaler1, AdcU::ClockSource::SystemClock,
					 AdcU::Prescaler::Disabled, AdcU::CalibrationMode::SingleEndedInputsMode);

	AdcV::initialize(AdcV::ClockMode::SynchronousPrescaler1, AdcV::ClockSource::SystemClock,
					 AdcV::Prescaler::Disabled, AdcV::CalibrationMode::SingleEndedInputsMode);

	AdcW::initialize(AdcW::ClockMode::SynchronousPrescaler1, AdcW::ClockSource::SystemClock,
					 AdcW::Prescaler::Disabled, AdcW::CalibrationMode::SingleEndedInputsMode);

	AdcU::connect<SenseU::In1>();
	AdcV::connect<SenseV::In12>();
	AdcW::connect<SenseW::In4>();
	...
	// AdcW::setChannel(AdcW::Channel::Channel2, AdcW::SampleTime::Cycles13);
